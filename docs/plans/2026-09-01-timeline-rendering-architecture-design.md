...
| **P5b** | 去掉滚轮缩放的 `flushSync`（命令式写内容层宽度）+ 标尺整体记忆化 | **已完成**（`5cae1f65`） |
...
占比完全未测。P5a / P5b 是按代码定位后去掉的（刻度重算、标尺子树协调、
每帧全树同步提交），但**它们的收益同样无法离线验证**。

**P5b 的已知风险**：缩放时 canvas 层在 rAF 内同步绘制，DOM 层由 React 异步
提交，二者是否仍在同一帧取决于 React 调度器。真机若出现画布与 DOM clip 分层，
说明该假设不成立，应回退 `5cae1f65`。
