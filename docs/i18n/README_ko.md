...
HiFiShifter는 세 가지 방법으로 미디어 파일을 가져올 수 있습니다. 동영상 파일은 자동으로 오디오 트랙을 사용합니다:

1. 시스템 파일 관리자에서 트랙으로 오디오 또는 동영상 파일을 직접 드래그 앤 드롭합니다.
2. 도구 모음의 폴더 아이콘을 클릭하여 내장 파일 브라우저를 열고 미디어 파일을 트랙으로 드래그합니다.
3. `Ctrl + F`를 눌러 빠른 검색을 열고 미디어 파일을 선택하여 트랙으로 가져옵니다(빠른 검색의 파일 경로는 내장 파일 브라우저의 현재 경로와 동일함).
...
- **복사/붙여넣기**: 클립을 선택하고 `Ctrl + C`를 누르면 애플리케이션 클립보드에 복사됩니다. `Ctrl + V`는 선택한 클립의 가장 왼쪽 시작 위치를 재생 헤드 위치에 맞추고 다른 클립의 상대적 간격을 유지합니다. 복사할 때 클립보드에 REAPERMedia 형식 데이터도 함께 기록되므로, REAPER에서 바로 붙여넣을 수 있습니다.
...
| 다중 선택 범위 (추가 / 해제)   | Ctrl + 드래그로 추가, Ctrl + 기존 범위 클릭으로 해제 (선택 모드) |
| 클립 범위를 파라미터 선택 범위에 추가 | Ctrl + Shift + A (또는 클립 오른쪽 클릭 메뉴) |
