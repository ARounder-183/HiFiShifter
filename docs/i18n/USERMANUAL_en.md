...
- Editing points: double-click a point flag on the Tempo Map row (including the initial point at 0), or right-click the ruler and choose `Edit This Point…`, to change the BPM (10-960), the time-signature numerator (1-32) / denominator (1/2/4/8/16/32) and the scale. Double-clicking a flag turns it into an inline text box where you can type text such as `120 4/4 - C / Am`; press `Enter` or click elsewhere to commit, `Esc` to cancel, and unrecognized input is silently discarded.
