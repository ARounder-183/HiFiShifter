...
HiFiShifter supports a project-level Tempo Map that lets you define different BPM, time signatures and scales at different positions of the timeline. A blank project has no Tempo Map data by default; once you add Tempo Map data to the project, the ruler automatically shows an extra Tempo Map row below the time units (separated by a short faint line), with each segment's tempo, time signature and scale shown on the blue point flags. When a segment's starting flag scrolls out of view on the left, a floating label in the same flag style appears at the far left of the row showing that segment's parameters; it switches smoothly while scrolling horizontally and never overlaps the flags. The parameter editor ruler shows the same Tempo Map row.

- Adding points: right-click the ruler and use `Add Tempo / Time Signature / Scale Change Here…` to create a point at the clicked position and open the edit dialog (BPM, time signature and scale in the dialog all support mouse-wheel adjustment; while holding the `Fine Adjust` modifier, the BPM wheel steps by 0.1); you can also double-click an empty area of the Tempo Map row.
- Editing points: double-click a point flag on the Tempo Map row, or right-click the ruler and choose `Edit This Point…`, to change the BPM (10-960), the time-signature numerator (1-32) / denominator (1/2/4/8/16/32) and the scale. The `Follow Previous Scale` option inherits the scale setting of the nearest point before this position (shown next to the option). The initial point (at 0) can be edited by double-click just like any other point.
- Initial point as the project record: the project's global BPM, beats per bar and scale are recorded at the initial point, shown on the Tempo Map row as e.g. `120 4/4 - C / Am`. Changing the project scale also updates the initial point's scale; editing the initial point's scale also updates the project scale.
- Moving / deleting points: any point except the first can be dragged horizontally (snapping applies when grid snap is enabled); `Delete This Point` in the right-click menu or the dialog removes a point; `Clear Tempo Map` removes all data and the project falls back to its global BPM / time signature / scale.
- Visibility: `View -> Tempo Map` (on by default) toggles the Tempo Map row. The row is hidden when the project has no Tempo Map data.
- Grid and ruler: when a Tempo Map exists, ruler ticks, bar/beat labels and the background grid re-align at every point and are computed per segment (non-uniform grid), and grid snapping follows the Tempo Map.
- Toolbar BPM / beats per bar / base scale: with a Tempo Map active, the toolbar shows the effective values at the playhead; editing BPM, beats per bar or the base scale updates the nearest point at or before the playhead (no point is created automatically). The base scale now represents that nearest point's scale setting rather than the project-wide scale.
