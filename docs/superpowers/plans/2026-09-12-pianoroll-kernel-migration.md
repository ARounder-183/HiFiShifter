...
1. **水平坐标域缺一段（Task 5）**：内核位置恒被钳到 `[0, max]`，而旧实现的
   **绘制**域是 `[−偏移, 内容宽]`（含负值）——同步留白无法表示，参数编辑器网格
   ...
   > **2026-09-13 修正（撤销「原生域含偏移」的一半）**：偏移是**纯投影量**，进了
   > 可滚范围就会让参数编辑器比轨道视图多出一段「只有自己能滚」的空白——滚到最右
   > 时轨道视图停在内容宽处（工程结尾落在轨道区左缘），参数编辑器还能再滚一个偏移量
   > （工程结尾落到轨道区左缘以左），两个面板从此不再对齐。现在参数编辑器内核的
   > **原生**域恒为 `[0, 内容宽]`（与轨道视图逐值相等，共享位置因此同域），偏移只在
   > 投影边界做 `drawing = native − 偏移` 的换算，不再注入 `extraContentWidthPx`
   > （绘制域因此是 `[−偏移, 内容宽 − 偏移]`）；可滚范围由两个面板共用的
   > `resolveTimelineScrollRange` 推出。
...
**3. 竖向缩放后位置算错（提交 `8b712409`）**

竖向缩放（钢琴键区 alt/ctrl+滚轮）只改 `span`，而视口存在 **ref** 里——ref 变更不触发
React 渲染，于是刷新「内核数据镜像」的 effect 不重跑，内核仍按**旧 span** 反算像素位置。
实测：alt+滚轮后 `nativeTop` 直接掉到 **0** 且再也动不了（旧实现正常平移到 554.5）。
修法：在提交前就地刷新镜像的 `valueDomain`。

反解验证（从观测到的 center 反推 span）：钢琴键区 alt+滚轮后
`top 554.313 / center 71.8965` → 解出 span = **21.5999** = `24 × 0.9`，
与竖向缩放的预期倍率逐值吻合；修复后与旧实现 554.5 相差 0.19px（量化）。

**该缺陷只有把「未覆盖的那条路径」也测到才会暴露**——此前测过的 alt+滚轮落在主画布上，
那条路径实际执行的是竖向**平移**（反解 span = 24.0001，未变），因此掩盖了 span 变更路径。

**4. 自绘滚动条拖拽未同步时间轴（收尾审计发现，提交 `44d34407`）**
