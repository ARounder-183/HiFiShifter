...
- **Dash phase must be anchored to the *data*, not the viewport.** *(Revised 2026-09-22 — the original guidance below reproduced a Canvas2D defect rather than describing intended behavior.)* `drawCurveTimed` skips samples before `viewportStartSec`, so the subpath — and hence the Canvas2D dash phase — restarts at the first visible sample every time `scrollLeft` changes; on screen this reads as the dashes "crawling" while scrolling. The GL path now receives `dashPhasePx` from `projectCurvePoints` (the first visible point's content-space distance from the curve's absolute start) and adds it to `v_along` via `u_dashPhase`, so the pattern is pinned to content; the Canvas2D fallback sets `ctx.lineDashOffset` from the same value. Historical note (why callers still pass only the visible slice): that remains a performance contract — projecting thousands of off-screen samples every frame is waste — not a correctness one.
