...
| 深色主题（已修） | 原取 `rgba(0,0,0,0.08)` 在近黑底（`#1f1f1f` = 31）上只能到 29，**Δ2 ≈ 不可见**（对照：深色主题弱网格线本身 Δ11，比带子还明显 = 背景带白做）。改为**提亮** `rgba(255,255,255,0.04)` → **Δ9**，与网格线同量级：既看得清行、又不淹没线（`15806a15`） |
...
3. ~~深色主题钢琴背景偏弱（Δ2）~~ **已修**：深色主题改用提亮带（Δ9），见上。
