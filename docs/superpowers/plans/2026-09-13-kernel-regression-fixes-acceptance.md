...
| 深色主题（**两阶段修正**） | ① 原取 `rgba(0,0,0,0.08)` 在近黑底（`#1f1f1f` = 31）上只能到 29，**Δ2 ≈ 不可见**（对照：深色主题弱网格线本身 Δ11，比带子还明显 = 背景带白做）。② 曾改为**提亮** `rgba(255,255,255,0.04)`（Δ9）——**这个方向是错的**：提亮让黑键行比白键行更亮，与键盘列（黑键恒暗于白键）**恰好相反**，用户报告"深色模式下背景的黑键和白键的深色区域是相反的（浅色模式正常）"。③ 最终修法：**保持压暗方向**、只把 alpha 加大到 `rgba(0,0,0,0.25)` → 带 23 vs 白键行 31，**Δ7.75**；仍在弱网格线 Δ11.2 之下（带内网格线实测 Δ12，未被淹没） |
| 浅色主题 | 压暗有余量：`rgba(0,0,0,0.06)` → 白键行 240 vs 带 226，**Δ14**；改动后复测不变 |
| **方向性回归守卫** | `colors.test.ts` 逐主题把带子合成到底色上，断言"黑键行更暗"（深色主题在提亮版本下必然失败）＋ Δ ≥ 6（挡"白做"）＋ Δ带 ≤ Δ最弱网格线（挡"淹没线"）；另断言两套主题的 `playheadLine` 都等于 `PLAYHEAD_COLOR_TOKEN` |
...
| 第二轮（用户真机 3 项） | **888** | **+8**（`colors.test.ts` 6 + `pianoRollKernelHost.test.ts` 2） |
| **实测终值** | **888 passed / 2 failed (890)** | **+50** |
...
> 用户在同一面板（参数编辑器 + 同步视图）上报告的三项，全部**在真机复现并定位到根因**后修复。
> 三项都不是"再调个色值"，其中 1 与 2 是**接线缺陷**（图层/参数没人喂）。

### 问题 1：同步视图下标尺刻度与网格线对不齐

| 项 | 结果 |
| --- | --- |
| 根因（**坐标空间不一致**） | `pianoRollKernelHost.syncDom` 拿 `view.scrollLeft`（**原生坐标**）去写标尺内容层与网格层。同一帧里 `currentAxis()`（投影）、`onFrame`（面板绘制）、量化提交三处都做了 `− offset` 换算，**唯独这一处漏了** |
| 实测（offset=200、原生 0） | 修复前：参数面板标尺内容坐标 0 落在屏幕 x **256**（与时间轴内容 0 重合），网格内容坐标 0 落在屏幕 x **56** —— 整差一个偏移量。修复后网格内容 0 → 屏幕 **256**，与两侧标尺（255）一致 |
| 为什么只在同步时可见 | offset 恒为 0 时两个坐标系重合，缺陷**不可见**；只有开启「同步时间轴视图」才暴露 |
| 跨滚动位置复验 | 原生 0 → 参数原生 400 → 800 三档：两侧标尺刻度屏幕 x 恒等（255/255 → −145/−145 → −545/−545），参数刻度数 = 时间轴刻度数（19/19） |
| **残差判定（1px 不是本轮引入）** | 刻度线是 1px、强网格线是 2px，两者居中约定不同（`TimeRulerMarks` 的小节线左移 1px 以自身位置为中心），故最近强线恒偏 1px。**关闭同步后**复测：刻度与网格的**内容坐标完全相同**（300 vs 300），屏幕仍差 1px —— 证明这 1px 是既有的线宽居中约定，不是残留偏移 |
| 为什么之前没被测到 | 单测的网格层桩只提供 `style`，而网格的重绘走 `invokeGridRedrawHandler(元素, 偏移)`、**不写任何 DOM 属性** → 该通道原本**不可观测**。已改为经 `setGridRedrawHandler` 注册记账回调 |
| 回归测试 | `pianoRollKernelHost.test.ts` 新增 2 例：标尺与网格必须收到**同一个**绘制坐标（= 原生 − 偏移）；偏移为 0 时两者仍一致（防"无条件 + 偏移"的过度修正） |

### 问题 2：参数面板播放线与上方标尺颜色不一致

| 项 | 结果 |
| --- | --- |
| 根因（**唯一绘制者没人喂颜色**） | 参数编辑器的 Canvas2D 播放头已被 `skipPlayhead: true` **永久跳过**，播放头只剩 GL 叠加层一个绘制者；而面板从不传 `playheadRgba`，宿主便用了硬编码兜底 `[0,0,0,0.2]`。配色表里的 `playheadLine` 因此是**死值** |
| 实测 | 修复前：标尺列 `#de4f5d`(222,79,93)、画布列 `140,157,177`（浅色主题下 0.2 黑叠在浅底上）。修复后：同一条播放头从标尺贯穿到画布底部**同一色** 222,79,93（y=128..2351） |
| 修复 | 配色表 `playheadLine` 改为唯一 token `PLAYHEAD_COLOR_TOKEN = "var(--qt-playhead)"`（与 `TimeRuler` 的 `bg-qt-playhead` 同源，用户在外观设置里改色两处一起变）；面板每帧把 `resolvePlayheadRgba()` 喂进 `overlay.playheadRgba` |
| 性能 | 解析要借 DOM 归一化 `var(...)`（GL 不认变量、`parseRgbaColor` 只认 `rgb()/rgba()`），而该块**每帧**执行 → 按解析后的字符串做缓存，稳态零重算、主题一改键即变 |
| 回归测试 | `colors.test.ts` 断言两套主题的 `playheadLine` 都等于 token（字面色值必然失败） |

### 问题 3：深色模式钢琴背景的明暗方向与键盘列相反

见上表「缺陷 6b · 深色主题（两阶段修正）」——这条是**上一轮我自己引入的**（`15806a15` 把深色改成提亮以解决"Δ2 不可见"，方向改反了）。最终修法是保持压暗、加大 alpha。

## 门禁

| 门禁 | 结果 |
| --- | --- |
| `npx vitest run` | **2 failed（既有）/ 888 passed (890)**，140 个测试文件（基线 882 → **+8**：`colors.test.ts` 6 例 + `pianoRollKernelHost.test.ts` 2 例） |
...
3. ~~深色主题钢琴背景偏弱（Δ2）~~ **已修两次**：先误改为"提亮"（方向错，与键盘列相反），最终保持压暗并把 alpha 提到 0.25（Δ7.75），见上。
