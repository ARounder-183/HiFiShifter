...
| `pianoRollKernel.gl`（静态场景层） | 移除开关，**保留运行期软着陆**（GL 失败 → Canvas2D 画得出来，这个回退是真有价值的） |
| `pianoRollKernel.curveGl`（曲线层） | 同上 |
| `drawPianoRoll`（Canvas2D 细节层） | **保留**，它是内核的一部分，不是旧路径 |

即：**"没有回退"只用在真的没有回退的地方**（时间轴的网格/clip）；参数编辑器内部 GL 与 Canvas2D 的分工是内核的**内部实现细节**，其失败自愈能力继续保留。
...
5. 参数编辑器在移除三个开关后行为不变；GL 层失败仍能软着陆到 Canvas2D。
