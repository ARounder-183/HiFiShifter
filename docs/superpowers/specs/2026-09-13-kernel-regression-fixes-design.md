...
| 7 | 打开「同步时间轴视图」后水平位置/缩放对不上、拖动抽搐 | 内核缺陷 | 两面板的**可滚域大小不同**（时间轴用 `projectSec`=120s，参数编辑器用 `getDynamicProjectSec`=59.5s），同步写入被浏览器钳制后永久错位 |
...
### 3.7 缺陷 7：同步时间轴视图时水平位置/缩放对不上、拖动抽搐

**用户报告**：打开「将参数编辑器的水平位置与缩放同步到时间轴」后完全对不上，水平拖动会抽搐。

#### 根因（已用运行时证据定位）

**两个面板的可滚域（scroll domain）大小不同**，同步时时间轴会把参数编辑器推到一个它**到不了**的位置，浏览器随即钳制，两边永久错位：

| 项 | 时间轴 | 参数编辑器 |
| --- | --- | --- |
| 时长来源 | `state.session.projectSec`（`TimelineKernelView:318`） | `getDynamicProjectSec(clips)`（`PianoRollPanel:1069`） |
| 实测值 | **120 s** | **59.5 s** |
| 内容宽 @150px/s | **18000 px** | 8925 px（spacer `paddedContentWidth` = 10989 px） |
| 可滚上限 | **18000 px**（`contentWidthPx()`，见 `scrollKernel` 的「水平上限语义」） | **9125 px**（浏览器语义 `scrollWidth − clientWidth`） |

实测（`?mock=1`，6 轨 / 24 clip，把时间轴拖到最右）：

```
BASE:            shared=0      paramMax=9125
AFTER_MAX_DRAG:  shared=18000  paramScrollLeft 被钳在 9125  → 两面板错位
```

以及标尺刻度的直接读数（同一时刻的 0s 标签屏幕 x）：拖到极限时 `timeLine = -636` 而
`pianoRoll = -761`，**相差 125 px**；快速来回拖动时差值冲到 **7650 px**，且
`shared` 在 `16687 ↔ 0` 之间来回跳——这正是"抽搐"。

**为什么"完全对不上"而不是只差一点**：120 s 与 59.5 s 相差一倍，参数编辑器在时间轴
还没滚到一半时就已到达自己的右端，之后所有同步写入都被钳制，于是越拖越偏。

#### 修复方向

二选一（**实现前先复现并确认哪一条更贴合既有设计意图**）：

1. **统一时长来源（首选）**：让参数编辑器与时间轴用**同一个**工程时长。二者本应一致
   ——`projectSec` 是后端给的工程全长，`getDynamicProjectSec` 只是"clip 末端"的兜底。
   建议时间轴侧改用与参数编辑器相同的一次求解并在两处共享（或反过来两处都用
   `projectSec`），**关键是唯一来源**。注意这会同时影响两边的 `paddedContentWidth` 与
   `maxScrollLeft`，必须一起核对。
2. **同步时按各自上限归一/钳制**：让共享视口承载"归一化位置"或让订阅方钳到自己的
   上限。**不推荐**：它掩盖了"两面板对工程长度理解不一致"这个真实缺陷，且归一化会
   让缩放锚点在两端失真。

**必须同时确认**：`resolveTimelineScrollRange` 的 `maxScrollLeft = contentWidth`（允许把
工程右端滚到视口左缘）与浏览器 `scrollWidth − clientWidth` 语义**本就不同**。即使时长
统一，参数编辑器也需要显式把 spacer 撑到 `contentWidth + 视口宽`，使
`scrollWidth − clientWidth` 等于 `contentWidth`——否则上限仍会差一个视口宽。

#### 验证

- 打开同步、把时间轴拖到最右：两面板标尺刻度的屏幕 x 差值必须为 0。
- 快速来回拖动（含方向反转）：`shared` 单调、无 `16687 ↔ 0` 跳变，差值恒为 0。
- 缩放（Ctrl+滚轮）后仍对齐。
