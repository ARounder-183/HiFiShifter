...
test("utils/timelineSnapping.test.ts scripted checks", async () => {
    let checks = 0;
    ...
    assertNear(
        snapStepBeats(
            { ...baseSettings, useIndependentSnapSpacing: true, snapSpacing: "1/8" },
            "1/4",
        ),
        0.5,
    ...
    // Swing 对齐现有 items。
    {
        const swingClips = [clips[0], { ...clips[1], id: "swing-c", startSec: 0.6 }];
    ...
    // (winEnd − b)/r 计算 —— 旧实现用正放公式会给出镜像错位的目标。
    {
        ...
        assertNear(
            snapTimelinePosition(ctx, 13.05).sec,
            13,
            "reversed media-start projects direction-aware",
        );
    ...
    // 双缘吸附（拖拽移动）：前缘与后缘同时参与匹配，取更近者。
    {
        ...
        assertTrue(
            startWin.snapped && startWin.edgeSide === "start",
            "start edge wins when closer",
        );
