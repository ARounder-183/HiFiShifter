...
// ── 非 Loop 正放：ss=2、se=6、len=4、r=1、媒体时长 D=8 ──
{
    ...
    // 非候选位置：两侧都不对齐。
    assertJson(
        slipBoundaryAlignedSides(clip, 1.5),
...
// ── Loop：len·r 恰为整周期 → 两侧同时对齐 ──
{
    ...
    assertJson(slipBoundaryAlignedSides(clip, 0), { start: true, end: true }, "loop period both");
}
...
console.log(`loopSnap checks passed (${checks})`);
