 ...
 * （真实输入会被误判为回声而吞掉）。
 *
 * 特殊说明 1：0.5 是 dpr 1 的**恰好上界**，`<=` 的闭区间语义在这里是承重的——
 * 改成 `<` 会让 dpr 1 边界情形漏判。`scrollEcho.test.ts` 用实测的三个 dpr 值钉住了
 * 两侧边界。
 *
 * 特殊说明 2（已知边界，未验证可达性）：dpr < 1（系统缩放低于 100%）时量化误差会
 * **超过** 0.5 —— 约 0.625 @0.8、0.746 @0.67、1.0 @0.5。此时落在该误差带内的真实
