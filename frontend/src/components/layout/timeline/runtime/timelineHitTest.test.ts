...
assertEqual(
    hitTestTimeline({ screenX: 148, screenY: 44, scrollLeftPx: 0, scrollTopPx: 0 }, offsetIndex),
    { trackId: "track-a", clipId: "clip-a", zone: "snap_offset" },
    "hit zone follows triangle at offset",
);
// 原左下角（三角已离开）：回落常规分区（0.05s 在 trim_left 阈值内）。
assertEqual(
    hitTestTimeline({ screenX: 105, screenY: 44, scrollLeftPx: 0, scrollTopPx: 0 }, offsetIndex),
    { trackId: "track-a", clipId: "clip-a", zone: "trim_left" },
...
assertEqual(
    hitTestTimeline({ screenX: 295, screenY: 44, scrollLeftPx: 0, scrollTopPx: 0 }, nearEndIndex),
    { trackId: "track-a", clipId: "clip-a", zone: "snap_offset" },
