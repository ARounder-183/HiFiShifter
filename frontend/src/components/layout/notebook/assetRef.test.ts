...
test("components/layout/notebook/assetRef.test.ts scripted checks", async () => {
    // ── 基本往返 ──────────────────────────────────────────────
    assertEqual(formatAssetRef("abc123", "webp", 640), "hifi-asset://abc123.webp#w=640", "full form");
    assertEqual(formatAssetRef("abc123"), "hifi-asset://abc123", "id only");
    assertEqual(formatAssetRef("abc123", "png", 0), "hifi-asset://abc123.png", "zero width omitted");
    ...
    assertEqual(src.slice(refs[0].start, refs[0].end), "hifi-asset://a1.webp#w=320", "range covers ref");
    ...
    assertEqual(scanAssetRefs("hifi-asset://x.png#w=")[0].width, undefined, "dangling width absent");
    ...
    assertEqual([...referencedAssetIds(markdown)].sort(), ["clip1", "img1"], "referenced ids deduped");
