...
import { redoRemote, undoRemote } from "./thunks/projectThunks.js";
...
test("features/session/sessionSlice.history.test.ts scripted checks", async () => {
    ...
    // ── 深度镜像 ──
    {
        const seeded = reducer(
            createState(),
            setHistoryDepths({ undoDepth: 3, redoDepth: 1 }),
        );
    ...
    {
        const seeded = reducer(createState(), setHistoryDepths({ undoDepth: 0, redoDepth: 2 }));
    ...
    {
        // 拉伸发生时深度 3 → 回写打点后深度 4。
        let state = reducer(createState(), setHistoryDepths({ undoDepth: 3, redoDepth: 0 }));
        state = reducer(
            state,
            recordParamSelectionStretchStep({
                positionBefore: 3,
        ...
        // 新检查点：深度 4。
        state = reducer(state, setHistoryDepths({ undoDepth: 4, redoDepth: 0 }));
    ...
    {
        let state = reducer(createState(), setHistoryDepths({ undoDepth: 2, redoDepth: 0 }));
        state = reducer(
            state,
            recordParamSelectionStretchStep({
                positionBefore: 1,
        ...
        state = reducer(state, setHistoryDepths({ undoDepth: 3, redoDepth: 0 }));
    ...
    // ── 新检查点截断重做分支：被丢弃分支上的步骤作废 ──
    {
        ...
        const truncated = reducer(
            undone,
            setHistoryDepths({ undoDepth: 4, redoDepth: 0 }),
        );
    ...
    // ── 历史清空（新建 / 打开工程）：深度归零且步骤全部作废 ──
    {
        ...
        state = reducer(state, setHistoryDepths({ undoDepth: 0, redoDepth: 0 }));
