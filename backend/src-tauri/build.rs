fn main() {
    ...
    // tauri_build validates resources.  SoundTouchDLL.dll / .so / .dylib are
    // built by build_soundtouch() above and must exist before this call.
    // Resource entries come from the committed tauri.windows.conf.json
    // (Windows) and tauri.macos.conf.json (macOS); for Linux AppImage the
    // config is generated by CI.  No dynamic config generation is needed here.
    tauri_build::build();
}
