//! SoundTouch 时间拉伸 FFI 封装
//!
//! 跨平台音频时间拉伸模块。
//! - Windows: 动态链接 SoundTouchDLL_x64.dll
//! - macOS/Linux: 静态链接编译自源码的 SoundTouch 库 (soundtouch_mac)
//!
//! 仅处理”保音高的时间拉伸”，因此固定使用：
//! - `rate = 1.0`
//! - `pitch = 1.0`
...
// SoundTouch FFI bindings.
// On Windows, these are provided by SoundTouchDLL_x64.dll.
// On macOS/Linux, these are provided by the statically-linked soundtouch_mac library.
#[cfg_attr(target_os = “windows”, link(name = “SoundTouchDLL_x64”))]
extern “C” {
...
/// SoundTouch is available on:
/// - Windows x86_64 (via DLL)
/// - macOS (via static library, both x86_64 and aarch64)
/// - Linux (via static library)
pub fn is_available() -> bool {
    if cfg!(all(target_os = "windows", target_arch = "x86_64")) {
        return true;
    }
    // On macOS/Linux, SoundTouch is statically linked if the build succeeded.
    // The build.rs will skip if sources aren't found, in which case we fall back
    // to Signalsmith Stretch at runtime.
...
mod tests {
    ...
    #[test]
    fn availability_on_current_platform() {
        // On Windows x86_64, SoundTouch should be available via DLL.
        // On macOS/Linux, SoundTouch should be available via static link.
        if cfg!(any(
