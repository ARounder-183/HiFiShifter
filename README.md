...
### Tempo Map（变速 BPM）
HiFiShifter 支持 Tempo Map 功能，允许在时间线上设置多个速度变化点，实现变速 BPM。

- **显示/隐藏速度轨**：点击工具栏 BPM 输入框旁的 **T** 按钮切换速度轨的显示/隐藏。
- **添加速度点**：在速度轨上双击即可在该位置添加新的速度变化点，新点继承当前位置的 BPM 和拍号。
- **编辑速度点**：双击速度标记（flag）打开弹窗，可编辑 BPM（10–300）和拍号（分子/分母）。
- **删除速度点**：鼠标悬停速度标记时出现删除按钮（首个速度点不可删除）。
- **BPM 输入框联动**：当速度轨可见时，在工具栏 BPM 输入框中输入数值会在播放头位置添加/更新速度点。
- **网格吸附**：变速模式下，时间线网格和吸附自动适配变速 bar 位置。
- **Clip 联动**：更新速度点后，受影响区域的音频切片 playback_rate 会自动联动调整。
- **工程保存/加载**：Tempo Map 数据随工程文件自动保存和恢复，兼容旧工程。
